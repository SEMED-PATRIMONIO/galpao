    ...
    const dispararRegistroPresenca = () => {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
            try {
                ...
                // Se o backend retornar erro (como fora do raio, fora do horário, etc.)
                if (!res.ok) {
                    setMsgErro(data.error || 'Erro ao processar presença.');
                    // Regra estrita: Alerta dura exatamente 8 segundos e encerra limpando tudo
                    setTimeout(() => {
                        setMsgErro('');
                        // Aqui você limpa estados ou redireciona para desconectar amigavelmente
                    }, 8000);
                    return;
                }

                // Tratamento das situações com base na inteligência do backend
                if (data.status === 'multiplos_eventos' || data.multiplos_eventos) {
                    setMultiplosEventos(data.eventos);
                    return;
                }

                if (data.status === 'necessita_saida') {
                    // Direciona o professor para o formulário de Pesquisa de Satisfação
                    setFluxoSaida({
                        tipo: 'pesquisa',
                ...
                if (data.status === 'confirmar_entrada') {
                    // Exibe em tela as informações do evento: Nome, Local e Endereço para clique de confirmação
                    setFluxoSaida({
                        tipo: 'confirmar_entrada',
                        evento_id: data.evento.id,
                        titulo: data.evento.titulo,
                        local: data.evento.local,
                        endereco: data.evento.endereco
                    });
                    return;
                }

                if (data.status === 'sucesso') {
                    setMsgSucesso(data.mensagem); // "REGISTRO EFETUADO!"
                    setTimeout(() => {
                        setMsgSucesso('');
                        setFluxoSaida(null);
                    }, 8000);
                ...
                setMsgErro('Não foi possível estabelecer contato com o portal de presença.');
                setTimeout(() => setMsgErro(''), 8000);
            }
        });
    };
