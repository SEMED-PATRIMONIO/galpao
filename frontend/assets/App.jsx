...
export default function App() {
    ...
    const verificarStatusAparelho = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:3009/api/v2/dispositivo/status?device_token=${deviceToken}`);
    ...
    const vincularAparelho = async (e) => {
        ...
        try {
            setMsgErro('');
            const res = await fetch('http://localhost:3009/api/v2/dispositivo/associar', {
                method: 'POST',
    ...
    const dispararRegistroPresenca = () => {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
            try {
                const res = await fetch('http://localhost:3009/api/v2/presenca/registrar', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const registrarEventoEspecifico = async (eventoId) => {
        try {
            const res = await fetch('http://localhost:3009/api/v2/presenca/registrar-especifico', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const processarSaidaPrecoce = async () => {
        try {
            const res = await fetch('http://localhost:3009/api/v2/presenca/confirmar-saida-precoce', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const processarSaidaDefinitiva = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch('http://localhost:3009/api/v2/presenca/concluir-saida', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div style={estilos.containerMobile}>
            <div style={estilos.wrapperMobile}>
                ...
                {view === 'identificacao' && (
                    <form onSubmit={vinterAparelho} style={estilos.formMobile}>
