...
export default function App() {
    ...
    const [modalSaidaManual, setModalSaidaManual] = useState({ visivel: false, horaInput: '' });
    ...
    // CORREÇÃO: Função melhorada utilizando apiFetch para envio do Token de Autorização e refresh da tabela
    const processarSaidaManual = async () => {
        if (!modalSaidaManual.horaInput) return alert('Digite a hora de saída.');

        try {
            const data = await apiFetch('/api/v2/admin/frequencias/saida-manual', {
                method: 'POST',
                body: JSON.stringify({ 
                    frequencia_id: selecionado.id, 
                    hora_saida: modalSaidaManual.horaInput 
                })
            });
            
            alert(data.message || 'Saída registrada com sucesso!');
            setModalSaidaManual({ visivel: false, horaInput: '' });
            setSelecionado(null);
            carregarDadosPainel(); // Recarrega a tabela imediatamente para atualizar a visualização
        } catch (error) {
            alert(error.message || 'Erro de conexão ao salvar saída manual.');
        }
    };
    ...
    return (
        <div style={estilos.layoutPrincipal}>
            <div style={estilos.barraLateral}>
                ...
                <div style={view === 'frequencias' ? estilos.menuItemAtivo : estilos.menuItem} onClick={() => { setView('frequencias'); setDadosEstatisticos(null); setIsEditando(false); setSelecionado(null); }}>HISTÓRICO</div>
...
{/* INTERFACE DE EVENTOS */}
                    ...
                    {/* MODIFICAÇÃO SOLICITADA: EXCEÇÃO DO HISTÓRICO COM BOTÃO DE SAÍDA MANUAL */}
                    {view === 'frequencias' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', height: '100%' }}>
                            {/* BARRA SUPERIOR EXIGIDA COM O BOTÃO À ESQUERDA DA CONTAGEM */}
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', padding: '12px 15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                    <button 
                                        disabled={!selecionado || selecionado.data_saida !== null} 
                                        onClick={() => setModalSaidaManual({ visivel: true, horaInput: '' })}
                                        style={{
                                            padding: '8px 15px', borderRadius: '6px', border: 'none', fontWeight: 'bold', fontSize: '12px',
                                            cursor: (!selecionado || selecionado.data_saida !== null) ? 'not-allowed' : 'pointer',
                                            backgroundColor: (!selecionado || selecionado.data_saida !== null) ? '#cbd5e1' : '#f59e0b',
                                            color: (!selecionado || selecionado.data_saida !== null) ? '#94a3b8' : '#fff'
                                        }}
                                    >
                                        ⚙️ REGISTRAR SAÍDA MANUALMENTE
                                    </button>
                                    
                                    {selecionado && (
                                        <span style={{ fontSize: '12px', color: '#475569', backgroundColor: '#e2e8f0', padding: '4px 10px', borderRadius: '4px' }}>
                                            Selecionado: <strong>{selecionado.participante_nome || selecionado.matricula}</strong>
                                        </span>
                                    )}
                                </div>
                            </div>
            ...
            {modalSaidaManual.visivel && (
                <div style={estilos.backdropVitrificado}>
                    <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '12px', width: '350px', textAlign: 'center', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
                        <h3 style={{ color: '#1e293b', marginTop: 0 }}>Saída Manual</h3>
                        <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '20px' }}>
                            Informe a hora de saída de <strong>{selecionado?.participante_nome || selecionado?.matricula}</strong>. <br/>
                            <em>Deve ser após a entrada e até o término oficial.</em>
                        </p>

                        <input 
                            type="time" 
                            value={modalSaidaManual.horaInput} 
                            onChange={(e) => setModalSaidaManual({ ...modalSaidaManual, horaInput: e.target.value })}
                            style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', marginBottom: '20px', boxSizing: 'border-box', fontSize: '18px', textAlign: 'center' }}
                        />

                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button type="button" onClick={() => setModalSaidaManual({ visivel: false, horaInput: '' })} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#fff', color: '#64748b', fontWeight: 'bold', cursor: 'pointer' }}>CANCELAR</button>
                            <button type="button" onClick={processarSaidaManual} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: 'none', backgroundColor: '#16a34a', color: '#fff', fontWeight: 'bold', cursor: 'pointer' }}>SALVAR SAÍDA</button>
                        </div>
                    </div>
                </div>
            )}
