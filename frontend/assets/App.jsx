...
export default function App() {
    ...
    // MECANISMO ANTI-FRAUDE: Coleta as propriedades de hardware do celular e monta um token fixo
    const obterFingerprintDispositivo = () => {
        const salvo = localStorage.getItem('device_token');
        if (salvo) return salvo;

        const n = window.navigator;
        const s = window.screen;
        const str = `${n.userAgent}-${s.width}x${s.height}-${n.language}`;
        const hash = btoa(unescape(encodeURIComponent(str))).replace(/[^a-zA-Z0-9]/g, '').substring(0, 32);
        
        const novoToken = `web-${hash}`;
    ...
    const [deviceToken, setDeviceToken] = useState(obterFingerprintDispositivo()); // Garante o fingerprint nativo fixo
...
// Definição da URL oficial do seu backend para sincronização remota
    ...
    const lidarComLimpezaGeral = () => {
        ...
        setProfessor(null);
        setDeviceToken(obterFingerprintDispositivo()); // Mantém rigidamente a mesma assinatura de hardware do aparelho
