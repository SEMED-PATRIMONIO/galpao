...
export default function App() {
  ...
  const enviarFeedbackESaida = () => {
    ...
    .then(res => {
      if (res.requere_matricula) {
        setPasso('pedir_matricula');
      } else if (res.requer_cadastro) {
        setMatriculaInput(res.matricula_tentada);
        setPasso('cadastro_novo');
      } else if (res.success === false) {
        setCarregando(false);
  ...
  const enviarCadastroRapido = (e) => {
    e.preventDefault();
    if (!nomeInput || !matriculaInput) return alert('Preencha os dados.');
    setCarregando(true);
    fetch(`${API}/cadastrar-participante`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome_completo: nomeInput, matricula: matriculaInput, device_key: deviceKey })
    })
    .then(r => r.json())
    .then(res => {
        if(res.success) {
            // Cadastro feito! Agora tenta bater o ponto de novo automaticamente
            verificarPresencaNoServidor(gps, matriculaInput);
        }
    });
  };
      ...
      <div className="w-full max-w-md bg-slate-800 p-8 rounded-3xl shadow-xl text-center border border-slate-700">
        ...
        {!carregando && (
          <>
            ...
            {passo === 'cadastro_novo' && (
                <form onSubmit={enviarCadastroRapido} className="space-y-4 text-left">
                    <h3 className="text-xl font-black text-center text-blue-400">Cadastro de Participante</h3>
                    <p className="text-xs text-slate-300 text-center mb-4">Sua matrícula não foi encontrada. Cadastre-se agora para registrar sua presença.</p>

                    <div>
                        <label className="text-xs font-bold text-slate-400">Matrícula</label>
                        <input type="text" className="w-full bg-slate-900 p-3 rounded-xl border border-slate-700 mt-1 text-slate-500" value={matriculaInput} readOnly />
                    </div>
                    <div>
                        <label className="text-xs font-bold text-slate-400">Nome Completo</label>
                        <input type="text" className="w-full bg-slate-950 p-3 rounded-xl border border-slate-700 mt-1 outline-none text-white" value={nomeInput} onChange={e => setNomeInput(e.target.value)} placeholder="Digite seu nome completo" required autoFocus />
                    </div>

                    <button className="w-full bg-emerald-600 text-white p-3 rounded-xl font-black tracking-wide mt-2">CADASTRAR E BATER PONTO</button>
                </form>
            )}
