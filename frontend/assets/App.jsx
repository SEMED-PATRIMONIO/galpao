        ...
        {aba === 'eventos' ? (
          <div>
            ...
            <form onSubmit={criarEvento} className="grid grid-cols-4 gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-10">
              ...
              <button className="col-span-4 bg-indigo-600 text-white font-bold py-3 rounded-xl mt-4">SALVAR NOVO EVENTO</button>
            </form>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y">
                  {eventos.map(ev => (
                    <tr key={ev.id} className="hover:bg-slate-50 text-sm">
                      <td className="p-4 font-bold">{ev.titulo}</td>
                      <td className="p-4">{new Date(ev.data_evento).toLocaleDateString('pt-BR')}</td>
                      <td className="p-4">{ev.carga_horaria}h</td>
          ...
          <div>
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-3xl font-black">Frequência MEC</h2>
                ...
                <tbody className="divide-y text-sm">
                  {presencas.filter(p => p.nome_completo.toLowerCase().includes(filtro.toLowerCase())).map(p => (
                    <tr key={p.id} className="hover:bg-indigo-50/50 transition">
                      <td className="p-4"><div className="font-bold">{p.nome_completo}</div><div className="text-xs text-slate-400">{p.matricula}</div></td>
                      <td className="p-4 font-medium">{p.titulo} <span className="text-[10px] bg-indigo-100 text-indigo-700 px-2 rounded-full">{p.carga_horaria}h</span></td>
                      <td className="p-4 text-slate-500">{new Date(p.data_assinatura).toLocaleString('pt-BR')}</td>
                      <td className="p-4 text-center"><button className="text-indigo-600 font-bold hover:underline">Certificado</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
