...
// --- ROTAS DE GESTÃO DA EQUIPE (Para o Painel 3005) ---

// Criar novo usuário da equipe (Senha criptografada)
appPainel.post('/api/equipe/registrar', async (req, res) => {
    ...
    try {
        await pool.query(
            'INSERT INTO aee_usuarios_equipe (nome, login, senha_hash) VALUES ($1, $2, $3)',
            [nome, login, hash]
        );
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: "Login já existe." }); }
});
