...
app.post('/contratos/salvar', verificarAutenticacao, async (req, res) => {
    ...
    try {
        ...
        console.error("Erro ao salvar contrato/parcelas:", e);
        res.status(500).json({ erro: "Erro ao salvar contrato: " + e.message });
    } finally {
        client.release();
    }
});
...
app.post('/contratos/atualizar', verificarAutenticacao, async (req, res) => {
    if (req.usuarioLogado.perfil !== 'OPERADOR') return res.status(403).json({ erro: "Acesso negado. Apenas operadores podem alterar contratos." });
    ...
    try {
        const busca = await pool.query(`
            SELECT id, numero_parcela, to_char(data_vencimento, 'YYYY-MM-DD') as data_vencimento, 
                   valor_previsto, status, to_char(data_pagamento, 'YYYY-MM-DD') as data_pagamento, 
                   valor_pago, juros_pagos 
            FROM parcelas WHERE contrato_id = $1 ORDER BY numero_parcela ASC
        `, [req.params.id]);
