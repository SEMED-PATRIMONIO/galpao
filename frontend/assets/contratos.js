...
router.get('/api/v2/admin-exclusivo/dashboard/contratos-setor', verificarTokenAdminExclusivo, async (req, res) => {
    try {
        ...
        // Regra de Prazo estabelecida: Vencimento em menos de 30 dias
        const DIAS_ALERTA = 30;

        // Query para coletar os dados estatísticos consolidados
        const queryEstatisticas = `
            SELECT 
                ...
                COUNT(CASE WHEN 
                    status = 'Ativo' 
                    AND data_vencimento <= CURRENT_DATE + INTERVAL '${DIAS_ALERTA} days'
                    AND data_vencimento >= CURRENT_DATE
                    AND id NOT IN (
                        SELECT contrato_id FROM contratos_cienca_alertas WHERE setor_id = $1
                    )
                THEN 1 END) as alertas_pendentes
            FROM contratos
            WHERE setor_id = $1;
        `;
        const resStats = await pool.query(queryEstatisticas, [setor_id]);

        // Query para buscar os contratos em alerta que ainda NÃO receberam ciência desse setor
        const queryContratosAlerta = `
            SELECT id, numero_contrato, objeto, fornecedor, data_vencimento, valor_parcela
            FROM contratos
            WHERE setor_id = $1
              AND status = 'Ativo'
              AND data_vencimento <= CURRENT_DATE + INTERVAL '${DIAS_ALERTA} days'
        ...
        `;
        const resAlertas = await pool.query(queryContratosAlerta, [setor_id]);

        return res.json({
            totais: resStats.rows[0],
            listaAlertas: resAlertas.rows
        });
    } catch (e) {
        console.error("Erro no dashboard de contratos:", e.message);
...
router.post('/api/v2/admin-exclusivo/contratos/declarar-ciencia', verificarTokenAdminExclusivo, async (req, res) => {
    try {
        ...
        if (!contrato_id) return res.status(400).json({ error: "Contrato ID não informado." });

        await pool.query(`
            INSERT INTO contratos_cienca_alertas (contrato_id, usuario_id, setor_id)
            VALUES ($1, $2, $3)
            ON CONFLICT (contrato_id, setor_id) DO NOTHING
        `, [contrato_id, usuario_id, setor_id]);

        return res.json({ success: true, message: "Ciência registrada com sucesso!" });
    } catch (e) {
        console.error("Erro ao registrar ciência do contrato:", e.message);
        return res.status(500).json({ error: "Falha operacional." });
    }
});
