...
app.post(['/api/v2/presenca', '/api/v2/presencas', '/api/v2/presenca/confirmar-entrada'], async (req, res) => {
    // Aceita tanto device_token quanto device_key para não quebrar o front
    ...
    try {
        if (!tokenEfetivo || !evento_id) {
            return res.status(400).json({ error: 'Identificação do aparelho ou evento ausente.' });
        }

        // 1. Busca e valida o dispositivo ativo mapeando o participante_id real
        const resDisp = await pool.query(
            'SELECT * FROM dispositivos WHERE device_token = $1 AND ativo = true', 
            [tokenEfetivo]
        );
        if (resDisp.rows.length === 0) {
            return res.status(401).json({ error: 'Aparelho não associado ou desativado pelo administrador.' });
        }
        
        // CORREÇÃO CIRÚRGICA: Alterado de resDev para resDisp para evitar o crash de ReferenceError
        const disp = resDisp.rows[0];

        // 2. Proteção contra duplicidade ativa em tempo real
        ...
        // 3. Gravação correta usando os campos exatos da sua tabela frequencias
        await pool.query(`
        ...
        `, [
            ...
            String(lngEfetiva || ''), 
            tokenEfetivo, 
            disp.participante_matricula
        ]);

        return res.json({ status: 'sucesso' });
    } catch (error) { 
        console.error('ERRO CRÍTICO NO INSERT DE PRESENÇA:', error);
        return res.status(500).json({ error: 'Erro interno ao salvar frequência.' }); 
    }
});

app.post(['/api/v2/presenca/confirmar-saida', '/api/v2/presenca/saida'], async (req, res) => {
    const { device_token, evento_id, estrelas, comentario, latitude, longitude } = req.body;

    console.log(`=> Tentativa de Saída Recebida! Evento: ${evento_id}`);

    try {
        if (!device_token || !evento_id) {
            return res.status(400).json({ error: 'Dados de identificação ausentes na saída.' });
        }

        // 1. Busca dispositivo e participante
        const resDisp = await pool.query('SELECT * FROM dispositivos WHERE device_token = $1 AND ativo = true', [device_token]);
        if (resDisp.rows.length === 0) return res.status(401).json({ error: 'Dispositivo inválido.' });
        const disp = resDisp.rows[0];

        // 2. Busca o evento e os registros de entrada para calcular o tempo real
        const resFreq = await pool.query(`
            SELECT f.*, e.hora_inicio, e.hora_fim, e.data_evento 
            FROM frequencias f 
            JOIN eventos e ON f.evento_id = e.id 
            WHERE f.participante_id = $1 AND f.evento_id = $2 AND f.data_saida IS NULL
            ORDER BY f.data_entrada DESC LIMIT 1
        `, [disp.participante_id, evento_id]);

        if (resFreq.rows.length === 0) {
            return res.status(404).json({ error: 'Nenhum registro de entrada em aberto encontrado.' });
        }
        const freq = resFreq.rows[0];

        // 3. Cálculo de tempo blindado (Respeitando regras de início/fim oficiais)
        const dataHoraReal = new Date();
        ...
        const dataHoraTerminoOficial = new Date(`${dataEventoFormatada}T${freq.hora_fim}`);
        ...
        if (formulaSaida > entradaParaCalculo) {
            const min = Math.floor((formulaSaida - entradaParaCalculo) / (1000 * 60));
            tempoFinalFormatado = `${String(Math.floor(min / 60)).padStart(2, '0')}:${String(min % 60).padStart(2, '0')}`;
        }

        // 4. Grava no banco de forma atômica
        ...
        await pool.query(`
            ...
            SET data_saida = $1, 
                lat_saida = $2, 
                lng_saida = $3, 
                tempo_participacao = $4 
            WHERE id = $5
        `, [dataHoraReal, String(latitude || ''), String(longitude || ''), tempoFinalFormatado, freq.id]);

        // Grava avaliação se enviada
        if (estrelas) {
            await pool.query(`INSERT INTO pesquisa_satisfacao (evento_id, participante_id, avaliacao, comentarios) VALUES ($1, $2, $3, $4)`, 
            [evento_id, disp.participante_id, String(estrelas), comentario || '']);
        }
        
        await pool.query('COMMIT');
        
        console.log(`[+] Saída registrada com sucesso. Tempo efetivo: ${tempoFinalFormatado}`);
        return res.json({ status: 'sucesso', tempo_gravado: tempoFinalFormatado });

    } catch (error) {
        await pool.query('ROLLBACK');
        console.error('ERRO CRÍTICO NA SAÍDA:', error);
