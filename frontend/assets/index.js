...
app.post('/api/verificar-localizacao', async (req, res) => {
    ...
    try {
        ...
        const travaDispositivo = await pool.query(
            "SELECT id FROM frequencias WHERE device_key = $1 AND matricula != $2 AND data_entrada > NOW() - INTERVAL '30 minutes' LIMIT 1",
        ...
        if (checkOpen.rows.length > 0) {
            ...
            // Processo de Checkin
            await pool.query(
                "INSERT INTO frequencias (participante_id, evento_id, data_entrada, device_key, matricula) VALUES ($1, $2, NOW(), $3, $4)",
