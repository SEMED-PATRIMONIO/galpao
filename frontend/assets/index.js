...
app.post('/api/v2/dispositivo/associar', async (req, res) => {
    // Aceita qualquer uma das propriedades que o React envie
    const { matricula, nome, device_token, token } = req.body;
    
    try {
        // Garante que temos um token válido para trabalhar
        const tokenDispositivo = device_token || token || uuidv4();
        
        // 1. Verifica se o dispositivo já está associado a alguém usando a coluna correta (device_token)
        ...
        // 3. Salva no banco usando APENAS as colunas que REALMENTE existem na sua tabela dispositivos
        await pool.query(`
        ...
        // 4. Busca os dados do participante cadastrado/localizado
        const partFinal = await pool.query('SELECT * FROM participantes WHERE id = $1', [participanteId]);
        
        // 5. TRUQUE DE COMPATIBILIDADE: Retorna o token com os dois nomes! 
        // O banco fica limpo e o React lê a propriedade 'token' ou 'device_token' sem se perder!
        return res.json({ 
