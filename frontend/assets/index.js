...
app.post('/api/v2/presenca/registrar', async (req, res) => {
    ...
    try {
        ...
        const resAberto = await pool.query(`
            SELECT f.*, e.titulo, e.horario_fim, e.latitude as ev_lat, e.longitude as ev_lng,
        ...
        if (resAberto.rows.length > 0) {
            ...
            const minutosFimPrevisto = paraMinutos(freqAtiva.horario_fim);
            ...
            if (estrelas) {
                ...
                `, [freqAtiva.evento_id, participanteId, estrelas, comentario || '']);
            }
