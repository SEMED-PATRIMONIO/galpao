...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        // Agora busca pela coluna 'usuario' e traz também o status 'deve_alterar_senha'
        const result = await pool.query(
            "SELECT id, nome, usuario, ativo, deve_alterar_senha FROM usuarios WHERE usuario = $1 AND senha = $2", 
            [usuario, senha]
        );
        ...
        if (result.rows.length > 0) {
            ...
            // Retorna o sucesso e se o usuário precisa alterar a senha
            res.json({ 
                success: true, 
                usuario: result.rows[0].usuario, 
                nome: result.rows[0].nome,
                deve_alterar_senha: result.rows[0].deve_alterar_senha 
            });
        } else {
...
app.post('/api/auth/alterar-senha', async (req, res) => {
    ...
    try {
        // Atualiza a senha e desmarca o gatilho de alteração obrigatória
        await pool.query(
