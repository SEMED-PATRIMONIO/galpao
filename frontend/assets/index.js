...
app.post('/api/v2/presenca/registrar', async (req, res) => {
    ...
    try {
        // 1. Valida se o dispositivo existe e está ativo
        ...
        if (resDisp.rows.length === 0) {
            return res.status(401).json({ error: 'Aparelho não autorizado ou desconectado.' });
        }
        const disp = resDisp.rows[0];
        const participanteId = disp.participante_id;

        // 2. Busca TODAS as formações/eventos ativos para a data de hoje
        ...
        if (resEventos.rows.length === 0) {
            return res.status(400).json({ 
                error: 'Não encontramos nenhuma formação agendada para o dia e horário de hoje.' 
            });
        }

        // 3. Filtra os eventos que estão dentro do raio de tolerância (Ex: 200 metros)
        ...
        const eventosProximos = resEventos.rows.filter(evento => {
            ...
            // CHAMA A SUA FUNÇÃO EXISTENTE NO ARQUIVO:
        ...
        // Se nenhum evento de hoje estiver perto geograficamente do professor
        if (eventosProximos.length === 0) {
            return res.status(400).json({ 
                error: 'Você não está próximo a nenhuma formação ativa no momento.' 
            });
        }

        // 4. Se houver MAIS DE UM evento próximo, envia a lista para o Front-end
        if (eventosProximos.length > 1) {
            return res.json({ 
                multiplos_eventos: true, 
                eventos: eventosProximos.map(e => ({ id: e.id, titulo: e.titulo }))
            });
        }

        // Se chegou até aqui, existe exatamente UM evento próximo
        const eventoSelecionado = eventosProximos[0];

        // 5. Verifica se o participante JÁ REGISTROU ENTRADA hoje neste evento específico
        const resFreq = await pool.query(`
            SELECT * FROM frequencias 
            WHERE participante_id = $1 
              AND evento_id = $2 
              AND data_entrada::date = CURRENT_DATE
        `, [participanteId, eventoSelecionado.id]);

        if (resFreq.rows.length > 0) {
            const freqItem = resFreq.rows[0];
            
            // Se ele já registrou entrada mas ainda não bateu a saída
            if (!freqItem.data_saida) {
                return res.status(400).json({ 
                    error: `Você já está logado na formação "${eventoSelecionado.titulo}". Não esqueça de acessar novamente este mesmo QRCode quando for sair!` 
                });
            } else {
                return res.status(400).json({ 
                    error: `Você já concluiu sua participação na formação "${eventoSelecionado.titulo}" hoje.` 
                });
            }
        }

        // 6. REGISTRA A ENTRADA (Sem necessidade de foto)
        await pool.query(`
        ...
        return res.json({ 
            mensagem: `Presença registrada com sucesso na formação: ${eventoSelecionado.titulo}! Seja bem-vindo.` 
        });
