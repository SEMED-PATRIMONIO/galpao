...
app.post('/api/v2/eventos', verificarToken, async (req, res) => {
    ...
    try {
        ...
        // VALIDAÇÃO: Impede conflito de horários no mesmo local e mesmo dia
        ...
        // Busca os dados geográficos oficiais do Local cadastrado
        ...
        // Grava todas as colunas que estavam ficando em branco
        const result = await pool.query(`
            ...
            RETURNING *
        `, [
            titulo, data_evento, carga_horaria, local_id, publico_alvo_id, hora_inicio, hora_fim,
        ...
        return res.status(500).json({ error: 'Erro interno ao salvar a formação.' });
    }
});
...
app.put('/api/v2/eventos/:id', verificarToken, async (req, res) => {
    ...
    const { titulo, data_evento, carga_horaria, local_id, publico_alvo_id, hora_inicio, hora_fim, palestrante } = req.body;
    try {
        // VALIDAÇÃO: Impede conflito de horários (ignorando o próprio evento atual que está sendo editado)
        const conflito = await pool.query(`
            SELECT id, titulo FROM eventos 
