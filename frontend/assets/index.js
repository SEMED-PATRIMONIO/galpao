...
app.post('/api/v2/presenca/inicializar', async (req, res) => {
    ...
    try {
        ...
        // Correção de colunas reais: removido 'endereco', mantido 'local_exibicao'
        const resEventos = await pool.query(`
            SELECT id, titulo, local_exibicao, latitude, longitude,
        ...
        // Janela de entrada permitida: 30 minutos antes do início até 30 minutos antes do fim
        const elegiveisHorario = resEventos.rows.filter(ev => {
            const minInicio = paraMinutos(ev.h_ini);
            const minFim = paraMinutos(ev.h_fim);
            return minutosAbsolutos >= (minInicio - 30) && minutosAbsolutos <= (minFim - 30);
        });
    ...
    try {
        ...
        if (dist > 60) {
            await pool.query(`
                INSERT INTO log_fraudes (matricula, evento_id, motivo, distancia_calculada)
                VALUES ($1, $2, $3, $4)
            `, [disp.participante_matricula, evento_id, `TENTATIVA_FORA_DO_RAIO (Distância: ${Math.round(dist)}m)`, dist]);
