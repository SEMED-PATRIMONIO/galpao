...
app.post('/api/v2/presenca/registrar', async (req, res) => {
    ...
    try {
        ...
        // CORRIGIDO: busca focada unicamente na coluna real device_token
        const resDisp = await pool.query(
            'SELECT * FROM dispositivos WHERE device_token = $1 AND ativo = true', 
            [tokenMapeado]
        );
        ...
        const resEventos = await pool.query(`
            SELECT *, horario_inicio::time as h_ini, horario_fim::time as h_fim 
            FROM eventos 
        ...
        const eventosNoHorario = eventosProximos.filter(e => e.horario_inicio <= agoraStr && e.horario_fim >= agoraStr);
        ...
        if (eventosNoHorario.length > 1) {
            return res.json({ multiplos_eventos: true, eventos: eventosNoHorario.map(e => ({ id: e.id, titulo: e.titulo })) });
        }

        const eventoAtual = eventosNoHorario[0];

        const resFrequenciasHoje = await pool.query(`
            SELECT * FROM frequencias WHERE participante_id = $1 AND data_entrada::date = CURRENT_DATE
        `, [participanteId]);

        const presencaAtiva = resFrequenciasHoje.rows.find(f => !f.data_saida);

        if (presencaAtiva && presencaAtiva.evento_id !== eventoAtual.id) {
            return res.status(400).json({ error: 'Operação impossível: Você possui uma entrada ativa em outro evento pendente de saída.' });
        }
        ...
        if (frequenciaExistente) {
            if (!frequenciaExistente.data_saida) {
                ...
                await pool.query(`UPDATE frequencias SET data_saida = CURRENT_TIMESTAMP, permanencia = to_char(CURRENT_TIMESTAMP - data_entrada, 'HH24:MI:SS') WHERE id = $1`, [frequenciaExistente.id]);
        ...
        await pool.query(`INSERT INTO frequencias (participante_id, evento_id, data_entrada) VALUES ($1, $2, CURRENT_TIMESTAMP)`, [participanteId, eventoAtual.id]);
        return res.json({ sucesso: true, mensagem: `Proximidade confirmada! Presença registrada com sucesso na formação: ${eventoAtual.titulo}. Seja bem-vindo.` });
