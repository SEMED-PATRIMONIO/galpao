...
app.post('/api/v2/dispositivo/associar', async (req, res) => {
    ...
    try {
        // Usa o token enviado ou gera um novo caso o professor esteja registrando pela primeira vez
        const tokenDispositivo = device_token || uuidv4();
        
        // 1. Verifica se o dispositivo já está associado a alguém (Colunada corrigida para device_token)
        ...
        // 2. Localiza ou cria o participante
        ...
        // 3. Salva ou Atualiza o vínculo do aparelho (Mapeando device_token, participante_id e participante_matricula)
        await pool.query(`
        ...
        // 4. Busca os dados finais do participante para responder ao Front-end
        ...
        // Log detalhado no terminal para você acompanhar qualquer erro de execução
...
app.post('/api/v2/presenca/registrar', async (req, res) => {
    ...
    try {
        // 1. VALIDAÇÃO DO DISPOSITIVO (Compatível com device_token e token como fallback)
        const resDisp = await pool.query(
            'SELECT * FROM dispositivos WHERE (device_token = $1 OR token = $1) AND ativo = true', 
            [device_token]
        );
        if (resDisp.rows.length === 0) {
            return res.status(401).json({ error: 'Aparelho não autorizado ou vínculo expirado. Por favor, vincule novamente.' });
        }
        const disp = resDisp.rows[0];
        const participanteId = disp.participante_id;

        // 2. BUSCA EVENTOS DO DIA (Filtra apenas pela DATA de hoje para avaliar os horários dinamicamente)
        const resEventos = await pool.query(`
            SELECT *, 
                   horario_inicio::time as h_ini, 
                   horario_fim::time as h_fim 
            FROM eventos 
            WHERE ativo = true AND data_evento = CURRENT_DATE
        `);

        if (resEventos.rows.length === 0) {
            return res.status(400).json({ error: 'Não há nenhuma formação agendada para a data de hoje.' });
        }

        // 3. FILTRAGEM POR LOCALIZAÇÃO (Fórmula de Haversine usando a função do seu arquivo)
        const RAIO_MAXIMO_METROS = 200; // Raio configurável de proximidade
        const eventosProximos = resEventos.rows.filter(evento => {
            if (!evento.latitude || !evento.longitude) return false;
            const distancia = calcularDistancia(latitude, longitude, parseFloat(evento.latitude), parseFloat(evento.longitude));
            return distancia <= RAIO_MAXIMO_METROS;
        });

        // RECONHECIMENTO GEOGRÁFICO: Se não houver nenhum evento no raio de proximidade
        if (eventosProximos.length === 0) {
            return res.status(400).json({ error: 'Não há nenhum evento de Formação ocorrendo próximo de sua localização nesta data e horário.' });
        }

        // 4. VERIFICAÇÃO DE EVENTOS ATIVOS AGORA OU NOS PRÓXIMOS MINUTOS
        const agoraStr = new Date().toTimeString().split(' ')[0]; // Formato "HH:MM:SS"
        
        // Filtra os eventos que já começaram ou estão prestes a começar
        const eventosNoHorario = eventosProximos.filter(e => {
            // Permite registrar presença se estiver no horário ou minutos antes (conforme lógica do seu sistema)
            return e.horario_inicio <= agoraStr && e.horario_fim >= agoraStr;
        });

        if (eventosNoHorario.length === 0) {
            return res.status(400).json({ error: 'Existe uma formação próxima, mas ela não está no horário permitido para registro.' });
        }

        // Se houver mais de um evento no mesmo local e horário, o Front-end recebe a lista para escolha
        if (eventosNoHorario.length > 1) {
            return res.json({ 
                multiplos_eventos: true, 
                eventos: eventosNoHorario.map(e => ({ id: e.id, titulo: e.titulo }))
            });
        }

        const eventoAtual = eventosNoHorario[0];

        // 5. BLOQUEIO DE SIMULTANEIDADE (Não pode estar em dois eventos ao mesmo tempo)
        const resFrequenciasHoje = await pool.query(`
            SELECT * FROM frequencias 
            WHERE participante_id = $1 AND data_entrada::date = CURRENT_DATE
        `, [participanteId]);

        const presencaAtiva = resFrequenciasHoje.rows.find(f => !f.data_saida);

        if (presencaAtiva && presencaAtiva.evento_id !== eventoAtual.id) {
            return res.status(400).json({ 
                error: 'Operação impossível: Você possui uma entrada ativa em outro evento pendente de saída. Não é permitido participar de dois eventos simultaneamente.' 
            });
        }

        // 6. TRATAMENTO DE ENTRADA JÁ CONFIRMADA (Lógica de permanência e Saída)
        const frequenciaExistente = resFrequenciasHoje.find(f => f.evento_id === eventoAtual.id);

        if (frequenciaExistente) {
            // Se ele já registrou a entrada e está acessando de novo (Ação de Saída)
            if (!frequenciaExistente.data_saida) {
                
                // REGRA DE AUDITORIA (Até 40 minutos após o término do evento)
                // Converte o fim do evento e o momento atual para minutos do dia para cálculo preciso
                const [hFim, mFim] = eventoAtual.horario_fim.split(':').map(Number);
                const [hAgora, mAgora] = agoraStr.split(':').map(Number);
                
                const minutosFimEvento = (hFim * 60) + mFim;
                const minutosAtual = (hAgora * 60) + mAgora;
                const limiteMinutos = minutosFimEvento + 40;

                // Se o horário atual passou mais de 40 minutos do fim planejado do evento:
                if (minutosAtual > limiteMinutos) {
                    // Grava na tabela de fraudes para auditoria futura antes de rejeitar
                    await pool.query(`
                        INSERT INTO log_fraudes (participante_id, evento_id, motivo, detalhe, criado_em)
                        VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)
                    `, [
                        participanteId, 
                        eventoAtual.id, 
                        'SAIDA_EXPIRADA', 
                        `Tentativa de saída realizada às ${agoraStr}. O evento encerrou às ${eventoAtual.horario_fim} (Limite de 40 minutos estourado).`
                    ]);

                    return res.status(400).json({ error: 'Prazo limite de 40 minutos após o encerramento do evento expirou. Esta tentativa foi registrada e enviada para a Auditoria analisar.' });
                }

                // Saída dentro do tempo regulamentar: registra sem burocracia
                await pool.query(`
                    UPDATE frequencias 
                ...
                return res.json({ mensagem: `Saída da formação "${eventoAtual.titulo}" registrada com sucesso!` });
            } else {
                return res.status(400).json({ error: 'Você já concluiu a sua participação nesta formação hoje.' });
            }
        }

        // 7. REGISTRO DE ENTRADA INICIAL (Sem necessidade de foto)
        // Pergunta de confirmação "Deseja participar?" deve ser gerenciada pelo Front ao receber os dados do evento
        await pool.query(`
            INSERT INTO frequencias (participante_id, evento_id, data_entrada)
            VALUES ($1, $2, CURRENT_TIMESTAMP)
        `, [participanteId, eventoAtual.id]);

        return res.json({ 
            sucesso: true,
            evento_identificado: true,
            titulo_evento: eventoAtual.titulo,
            mensagem: `Proximidade confirmada! Presença registrada com sucesso na formação: ${eventoAtual.titulo}. Seja bem-vindo.` 
        });

    } catch (error) {
        console.error("Erro crítico na rota /presenca/registrar:", error.message);
