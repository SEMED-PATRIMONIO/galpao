  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9]">
      ...
      <main className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {stats ? (
          <>
            <CardKPI titulo="Alunos Matriculados" valor={stats.alunos || 0} icon="🎓" onClick={() => {}} />
            <CardKPI titulo="Total de Agendamentos" valor={stats.agendamentos || 0} icon="📅" onClick={() => {}} />
            <CardKPI titulo="Atendimentos Realizados" valor={stats.atendimentos || 0} icon="🩺" onClick={() => {}} />
            <CardKPI titulo="Especialidades" valor={stats.especialidades || 0} icon="🎗️" />
            <CardKPI titulo="Profissionais Ativos" valor={stats.profissionais || 0} icon="👥" />
            <CardKPI 
              titulo="Destaque do Período" 
              valor={stats.top_profissional?.nome || 'N/A'} 
              subtitulo={`${stats.top_profissional?.total || 0} atendimentos realizados`} 
              icon="🏆" 
            />
          </>
        ) : (
           <p className="col-span-3 text-center text-slate-500">Carregando estatísticas...</p>
        )}
      </main>
