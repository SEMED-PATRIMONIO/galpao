...
function abrirModalNovo() {
    const modal = document.getElementById('modalEstoque');
    const corpo = document.getElementById('corpoModal');
    const abaAtiva = document.querySelector('.tab-btn.active').innerText.trim();
    document.getElementById('modalTitulo').innerText = "Novo " + abaAtiva;
    corpo.innerHTML = `<input type="text" id="novo-nome" placeholder="Nome">`;
    if(abaAtiva.includes('Estoque')) corpo.innerHTML += `<input type="number" id="novo-qtd" placeholder="Qtd Inicial" value="0">`;
    
    document.getElementById('btnConfirmarGeral').onclick = async () => {
        const nome = document.getElementById('novo-nome').value;
        const tabela = abaAtiva.includes('Estoque') ? 'item' : (abaAtiva.includes('Locais') ? 'local' : 'categoria');
        await fetch(`${LOCAL_API_URL}/cadastrar/${tabela}`, {
            method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ nome, quantidade: document.getElementById('novo-qtd')?.value || 0 })
        });
        fecharModal(); carregarDados(tabela);
    };
    modal.classList.remove('hidden');
}
...
async function excluirItemLogico() {
    ...
    try {
        ...
        if (res.ok) {
            // Identifica qual era o índice do item atual antes de recarregar
            ...
            // Reposiciona no item anterior
            setTimeout(() => {
                const novasLinhas = document.querySelectorAll('.data-table tbody tr');
                if (novasLinhas.length > 0) {
                    // Tenta selecionar o anterior (indexAtual - 1), ou o primeiro se não houver anterior
                    const novoIndex = Math.max(0, indexAtual - 1);
                    const linhaParaSelecionar = novasLinhas[novoIndex];
                    if (linhaParaSelecionar) {
                        linhaParaSelecionar.click();
                        linhaParaSelecionar.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    }
                }
            }, 100); // Pequeno delay para garantir que o DOM foi montado
        } else {
