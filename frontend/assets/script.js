...
async function carregarDados(aba) {
    ...
    try {
        ...
        if (aba === 'item') dados = dados.filter(i => i.status === 'A' || !i.status); 
        // ABA: ESTOQUE (ITEM)
        if (aba === 'item') {
...
// --- FUNÇÃO PARA INATIVAR (DELETAR) ITEM ---
async function inativarItem() {
    if (!idSel) return;

    try {
        // Zera a quantidade e muda o status para 'I'
        const res = await fetch(`${LOCAL_API_URL}/item/${idSel}`, {
            ...
            body: JSON.stringify({ 
                status: 'I', 
                quantidade: 0,
                responsavel: JSON.parse(localStorage.getItem('session')).usuario 
            })
        });

        if (res.ok) {
            idSel = null; // Limpa seleção
...
// --- FUNÇÃO PARA ABRIR MODAL DE REATIVAÇÃO (VITRIFICADO) ---
async function abrirModalReativar() {
    ...
    // Aplica o efeito vitrificado dinamicamente ao modal
    modal.style.backdropFilter = "blur(15px)";
    modal.style.backgroundColor = "rgba(255, 255, 255, 0.4)";
    titulo.innerText = "TRAZER DE VOLTA (ITENS INATIVOS)";
    
    corpo.innerHTML = '<p style="text-align:center">Buscando itens arquivados...</p>';
    modal.classList.remove('hidden');

    try {
        // Busca todos os itens (ajuste a rota se seu backend tiver uma específica para inativos)
        const res = await fetch(`${LOCAL_API_URL}/item`);
        const dados = await res.json();
        
        // Filtra apenas os inativos e ordena alfabeticamente
        const inativos = dados
            .filter(i => i.status === 'I')
            .sort((a, b) => a.item.localeCompare(b.item));

        if (inativos.length === 0) {
            corpo.innerHTML = '<p style="padding:20px; text-align:center; color:#666;">Nenhum item para restaurar.</p>';
            btnConfirmar.classList.add('hidden');
        } else {
            let options = inativos.map(i => `<option value="${i.id}">${i.item}</option>`).join('');
            corpo.innerHTML = `
                <div style="padding:10px;">
                    <label style="display:block; margin-bottom:10px; font-weight:bold;">Selecione o item para reativar:</label>
                    <select id="select-reativar" style="width:100%; padding:12px; border-radius:8px; border:1px solid rgba(0,0,0,0.1); background:rgba(255,255,255,0.8);">
                        ${options}
                    </select>
                </div>
            `;
            ...
            btnConfirmar.onclick = async () => {
                const idParaReativar = document.getElementById('select-reativar').value;
                await confirmarReativacao(idParaReativar);
            };
        }
    } catch (err) {
        corpo.innerHTML = '<p style="color:red">Erro ao carregar lista.</p>';
    }

    // Configura o botão de fechar do seu modal padrão como o "CANCELAR"
    // Caso precise de um botão específico de cancelar dentro do corpo:
    const btnFechar = document.querySelector('.close-modal') || { onclick: () => {} };
}

// --- FUNÇÃO PARA CONFIRMAR A VOLTA DO ITEM ---
async function confirmarReativacao(id) {
    try {
        ...
        if (res.ok) {
            fecharModal();
            // Reseta o estilo do modal para não afetar os outros
            const modal = document.getElementById('modalEstoque');
