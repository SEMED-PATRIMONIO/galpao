...
async function inativarRegistro() {
    if (!idSel) return;
    // Executa a mudança diretamente sem confirmação, zerando a quantidade e mudando status para 'I'
    const res = await fetch(`${LOCAL_API_URL}/item/${idSel}/ajuste-estoque`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ novaQuantidade: 0, status: 'I' })
    });
    if (res.ok) {
        idSel = null; // Limpa a seleção
        carregarDados('item'); // Recarrega a lista principal
    }
}
