...
async function carregarDados(aba) {
    ...
    // Reset inicial: Esconde cabeçalhos fixos e os botões de ação
    ...
    try {
        ...
        if (aba === 'item') {
            ...
            // MOSTRA os botões de ação apenas nesta aba
            if (actions) actions.classList.remove('hidden');

            let html = '<table class="data-table"><tbody>';
            dados.forEach(i => {
                const isBaixo = i.quantidade <= i.alerta;
                html += `<tr onclick="selecionarLinha(this, ${i.id}, '${i.item}', ${i.quantidade}, ${i.alerta})" class="${isBaixo ? 'estoque-baixo-row' : ''}">
                    ...
                    <td style="width:6%; text-align:center">
                        <i class="fas fa-chevron-right"
                           ...
                           onclick="event.stopPropagation(); abrirPanorama('${i.item}')">
                        </i>
                    </td>
                </tr>`;
            });
        ...
        // ABA: HISTÓRICO
            ...
            // Botões de ação permanecem escondidos aqui
            
            let html = '<table class="data-table"><tbody>';
            dados.forEach(h => {
                const dataFmt = new Date(h.data).toLocaleDateString('pt-BR');
                const classeTipo = h.tipo === 'AJUSTE' ? 'tipo-ajuste' : '';
                html += `<tr>
                    <td style="width:12%">${dataFmt}</td>
                    <td style="width:25%">${h.nome_item}</td>
                    <td style="width:15%; text-align:center"><span class="${classeTipo}">${h.tipo}</span></td>
                    <td style="width:10%; text-align:center">${h.quant}</td>
                    <td style="width:13%; text-align:center">${h.num_nota || '-'}</td>
                    <td style="width:25%; text-align:center">${h.destino}</td>
                </tr>`;
            });
            main.innerHTML = html + '</tbody></table>';
        }
        // ABAS: CATEGORIAS E LOCAIS
        else {
            // Cabeçalhos e botões de ação permanecem escondidos aqui
            let html = `<table class="data-table"><tbody>`;
            dados.forEach(d => { 
                // d.nome é garantido pelo uso do COALESCE no backend
                html += `<tr><td style="padding:15px; border-bottom:1px solid #eee"><i class="fas fa-tag"></i> ${d.nome}</td></tr>`; 
            });
            main.innerHTML = html + '</tbody></table>';
        }
    } catch (err) { 
...
async function abrirModalNovo() {
    ...
    titulo.innerText = "NOVO REGISTRO DE PRODUTO";
    
    try {
        // Busca as categorias reais do seu banco
        const resCat = await fetch(`${LOCAL_API_URL}/categoria`);
        ...
        corpo.innerHTML = `
            <div style="display: flex; flex-direction: column; gap: 15px; padding: 10px;">
                <div>
                    ...
                    <input type="text" id="novo-nome" placeholder="Ex: PAPEL A4" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px; text-transform: uppercase;">
                </div>
                
                <div>
                    <label style="display:block; font-weight:bold; margin-bottom:5px;">CATEGORIA:</label>
                    <select id="novo-categoria" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px;">
                        ${categorias.map(c => `<option value="${c.id}">${c.nome_categoria}</option>`).join('')}
                    </select>
                </div>
                
                <div>
                    <label style="display:block; font-weight:bold; margin-bottom:5px;">ALERTA DE ESTOQUE MÍNIMO:</label>
                    <input type="number" id="novo-alerta" value="5" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px;">
                </div>
                <p style="color: #666; font-size: 0.85rem; font-style: italic;">* O saldo inicial será definido como zero. Use o botão 'ENTRADA' após criar.</p>
            </div>
        `;

        btnConfirmar.innerText = "SALVAR PRODUTO";
        btnConfirmar.onclick = salvarNovoItem;
        modal.classList.remove('hidden');
    } catch (err) {
...
async function salvarNovoItem() {
    const item = document.getElementById('novo-nome').value.trim();
    const categoria_id = document.getElementById('novo-categoria').value;
    const alerta_minimo = document.getElementById('novo-alerta').value;
    ...
    try {
        ...
        if (res.ok) {
            fecharModal();
            await carregarDados('item'); // Atualiza a lista na tela
...
async function excluirItemLogico() {
    if (!idSel) {
        alert("Selecione um item primeiro!");
        return;
    }

    if (!confirm(`Deseja realmente excluir o item: ${itemNomeSel}?`)) return;

    try {
        // O SEGREDO ESTÁ AQUI: Adicionar o method: 'PATCH'
        const res = await fetch(`${LOCAL_API_URL}/item/${idSel}/desativar`, {
            method: 'PATCH',
            headers: {
        ...
        if (res.ok) {
            // Lógica para pegar o índice atual antes de recarregar
            const linhas = Array.from(document.querySelectorAll('.data-table tbody tr'));
            const indexAtual = linhas.findIndex(tr => tr.classList.contains('selected'));
            
            await carregarDados('item');

            // Reposicionar no registro anterior
            setTimeout(() => {
                const novasLinhas = document.querySelectorAll('.data-table tbody tr');
                if (novasLinhas.length > 0) {
                    const novoIndex = Math.max(0, indexAtual - 1);
                    if (novasLinhas[novoIndex]) {
                        novasLinhas[novoIndex].click();
                        novasLinhas[novoIndex].scrollIntoView({ behavior: 'smooth', block: 'center' });
                    }
                }
            }, 150);
        } else {
...
function abrirModalEditar() {
    ...
    corpo.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 15px; padding: 10px;">
            <div>
                <label style="display:block; font-weight:bold; margin-bottom:5px;">NOME DO PRODUTO:</label>
                <input type="text" id="edit-nome" value="${itemNomeSel}" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px; text-transform: uppercase;">
            </div>
            <div>
                <label style="display:block; font-weight:bold; margin-bottom:5px;">ALERTA MÍNIMO:</label>
                <input type="number" id="edit-alerta" value="${alertaSel}" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px;">
            </div>
        </div>
    `;

    btnConfirmar.innerText = "SALVAR ALTERAÇÕES";
    btnConfirmar.onclick = salvarEdicaoItem;
    modal.classList.remove('hidden');
}

async function salvarEdicaoItem() {
    const novoNome = document.getElementById('edit-nome').value.trim();
    const novoAlerta = document.getElementById('edit-alerta').value;
    ...
    try {
        const res = await fetch(`${LOCAL_API_URL}/item/${idSel}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ novoNome, novoAlerta })
        });
        ...
        if (res.ok) {
            fecharModal();
            // Atualiza o nome na variável global para não dar erro se o usuário clicar de novo
            itemNomeSel = novoNome.toUpperCase(); 
            await carregarDados('item'); 
            alert("Item atualizado com sucesso!");
        } else {
            alert("Erro: " + (data.error || "Erro ao atualizar."));
        }
    } catch (err) {
        console.error(err);
