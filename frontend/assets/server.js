...
app.post('/api/item', async (req, res) => {
    const { item, categoria_id, alerta_minimo } = req.body;
    try {
        // quantidade nasce como 0 (default do banco) e status como 'A'
        const result = await pool.query(
            "INSERT INTO public.item (item, categoria_id, alerta_minimo, status) VALUES ($1, $2, $3, 'A') RETURNING *",
            [item.toUpperCase(), categoria_id, alerta_minimo || 5]
        );
...
// ROTA AUXILIAR PARA LISTAR CATEGORIAS NO SELECT
app.get('/api/categoria', async (req, res) => {
    try {
        const result = await pool.query("SELECT id, nome_categoria FROM public.categoria WHERE status = 'A' ORDER BY nome_categoria ASC");
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.put('/api/item/:id', async (req, res) => {
    const { id } = req.params;
    const { novoNome, novoAlerta } = req.body; // Podemos aproveitar para editar o alerta também
    try {
        const result = await pool.query(
            "UPDATE public.item SET item = $1, alerta_minimo = $2 WHERE id = $3 RETURNING *",
            [novoNome.toUpperCase(), novoAlerta, id]
        );
