...
// Servir os arquivos estáticos das pastas corretas
app.use(express.static('public_quiz')); 
app.use('/admin', express.static('public_dash')); 
...
// 3. ROTA: Salvar/Atualizar Inscrição (CORRIGIDO: appQuiz -> app)
app.post('/salvar', async (req, res) => {
    ...
    try {
        ...
        console.error("ERRO AO SALVAR NO POSTGRES:", err.message);
...
// 5. ROTA: Gerar PDF Geral (Todas as Fichas)
    ...
    try {
        ...
        rows.forEach(r => {
            ...
            htmlContent += `
                <div class="ficha">
                    <div class="header">
                        <h1 style="font-size:16pt;">OLIMPÍADA DE MATEMÁTICA ESTUDANTIL DE QUEIMADOS</h1>
                    ...
                    <table>
                        <thead><tr><th width="30">Nº</th><th>Nome do Aluno</th></tr></thead>
                        <tbody>
                            ${lista.map((n, i) => n ? `<tr><td align="center">${i+1}</td><td>${n}</td></tr>` : '').join('')}
                        </tbody>
                    </table>
                    <div class="footer">IP: ${r.ip_endereco} | Data: ${new Date(r.data_hora_envio).toLocaleString()}</div>
                </div>`;
        });
